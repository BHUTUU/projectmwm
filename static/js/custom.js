...
function deleteSelectedEntries(){
    $('#deleteSelectedEntries').modal('show');
    let checkboxes= document.querySelectorAll('input[name="u_id_delete"]:checked');
    let output= [];
    checkboxes.forEach((checkbox) => {
        output.push(checkbox.value);
    });

    console.log("chkId: ",output);

    $('#deleteEntriesField').val(output);
}
...
function finalDeleteSelectedEntries(){
    let entryid = $('#deleteEntriesField').val();
      $.ajax({
        url: baseurl + 'admin/deleteSelectedEntries', 
        type: "POST",             
        data: {'entryid':entryid},      
        success: function(e) {
            console.log("e: ", e);
            if(e == 0)
                ...
                showNotifications('success','Entries deleted successfully.');
                var userid=$('#deleteEntriesUserId').val();
              if(userid !="0"){
                setTimeout(function(){
                    window.location = baseurl+'admin/client_profile/'+userid;
                },3000);
              }else{
                setTimeout(function(){
                    window.location = baseurl+'admin/entries';
